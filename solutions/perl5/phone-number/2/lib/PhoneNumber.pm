...
sub clean_number($number) {
    ...
    die "letters not permitted" if ($number =~ m/\p{Alpha}/);
    die "punctuations not permitted" if ($number =~ m/[^\s\d\w\(\)\.\-\+]/);
    die "must not be fewer than 10 digits" if (scalar(@digits) < 10);
    die "must not be greater than 11 digits" if (scalar(@digits) > 11);
